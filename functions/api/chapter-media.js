...
async function requireAdmin(request) {
  ...
  const profileRes = await fetch(SUPABASE_URL + "/rest/v1/profiles?select=role,status&user_id=eq." + encodeURIComponent(user.id) + "&limit=1", { headers });
  if (!profileRes.ok) throw new Error("FORBIDDEN");
  const profiles = await profileRes.json();
  if (!profiles[0] || profiles[0].role !== "admin" || (profiles[0].status && profiles[0].status !== "active")) throw new Error("FORBIDDEN");
  return user;
}

export async function onRequestPost(context) {
  try {
    if (!context.env.CHAPTER_AUDIO) return json({ error: "Kho audio chưa được kết nối với Cloudflare R2." }, 503);
    ...
    if (!/^[0-9a-f-]{36}$/i.test(chapterId) || !context.request.body) return json({ error: "Dữ liệu tải lên không hợp lệ." }, 400);
    const max = kind === "audio" ? 95 * 1024 * 1024 : 8 * 1024 * 1024;
    if (!size || size > max) return json({ error: kind === "audio" ? "File audio phải nhỏ hơn 95MB." : "Ảnh phải nhỏ hơn 8MB." }, 413);
    ...
    await context.env.CHAPTER_AUDIO.put(key, context.request.body, { httpMetadata: { contentType: type, cacheControl: "public, max-age=31536000, immutable" } });
    ...
    return json({ error: "Không tải được file lên kho audio." }, 500);
  }
}
